...
#'
#' A helper function that uses \code{tidycensus::get_estimates()} to simplify
#' fetching U.S. Census Bureau Population Estimates Program (PEP) data.
#' Allows filtering by age group, race, sex, and Hispanic origin, and
#' optionally calculates percentages of the total population.
#'
#' @param vintage PEP vintage year (e.g., \code{2022}).
#' @param year Year within the vintage.
#' @param breakdown The population breakdown(s) used when 
...
#' @param age_group Either character keywords (e.g., \code{"65plus"}, 
#'   \code{"18to64"}) or numeric AGEGROUP codes (0–18) as defined 
#'   by the U.S. Census Bureau.
#' @param percent Logical; if \code{TRUE}, also include percentage 
#'   of total population.
#' @param ... Additional arguments passed to 
#'   \code{tidycensus::get_estimates()}.
#'
#' @return A tibble with \code{GEOID}, \code{NAME}, \code{population}, 
#'   and, if requested, \code{percent}.
#'
#' @import tidycensus
#' @export

get_population_age <- function(vintage   = 2021,
                               year      = 2021,
                               breakdown="AGEGROUP",
                               age_group = "65plus",
                               ...){
  ...
  if(length(group_vars)==0){
    pop_group <- pop_est %>%
     ...
     dplyr::group_by(GEOID, NAME) %>%
      dplyr::summarise(population = sum(value), .groups = "drop")%>%rename(county_fip=GEOID)
  } else {
    pop_group <- pop_est %>%
      dplyr::filter(AGEGROUP %in% codes) %>%
